...
class Decoder(object):
    ...
    def _to_int(self):
        ...
        end_token_index = self._payload.index(EOT_IDENTIFIER, self._index)

        # read whatever is between 'i' and 'e' and cast that to an int
        _int = int(self._payload[self._index:end_token_index])
    ...
    def _to_string(self):
        ...
        str_delim_index = self._payload.index(STRING_DELIMITER, self._index)

        # number of characters to read
        chars_to_read = int(self._payload[self._index:str_delim_index])

        # set the starting index at delimiter + 1 (ie the char after :)
        start_index = str_delim_index + 1

        # read `char_to_read` characters starting from delimiter index
        data = self._payload[start_index:start_index + chars_to_read]
